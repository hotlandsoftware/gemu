...
void pokey_write(Pokey *p, uint8_t reg, uint8_t v) {
    ...
    case 0x0D:                             /* SEROUT - no attached SIO target */
        ...
        p->irq_pending |= (uint8_t)(POKEY_IRQ_SEROR & p->irqen);
        p->serout_delay = 4;
        break;
...
void pokey_tick(Pokey *p) {
    ...
    if (p->serout_delay && --p->serout_delay == 0)
        p->irq_pending |= (uint8_t)(POKEY_IRQ_SEROC & p->irqen);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (--p->timer_count[i] == 0) {
            p->irq_pending |= (uint8_t)(irq_bits[i] & p->irqen);
...
void pokey_key_down(Pokey *p, uint8_t code) {
    p->kbcode = code;
    p->irq_pending |= (uint8_t)(POKEY_IRQ_KEY & p->irqen);
...
void pokey_break_key(Pokey *p) {
    p->irq_pending |= (uint8_t)(POKEY_IRQ_BREAK & p->irqen);
}
