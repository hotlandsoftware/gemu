...
typedef struct {
    ...
    uint32_t    addr;     /* CPU load address */
 ...
 * Calls fn(path, addr, ud) for each match; fn returns false to stop early.
 * Returns number of successful fn calls, or -1 on directory open error. */
typedef bool (*RomDbAddFn)(const char *path, uint32_t addr, void *ud);
